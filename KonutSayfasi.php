...
if($_REQUEST["MenuID"]!=""){
    ...
    $MenuKosulu  = " AND odasayisi = '" . $GelenMenuID . "'";
...
$ToplamKayitSayisiQuery             = $DataBaseConnection->query("SELECT * FROM konut WHERE Durumu = 1 $MenuKosulu $AramaKosulu");
$KacTaneEvVar                       = $ToplamKayitSayisiQuery->num_rows;
    
$SayfalamayaBaslanacakKayitSayisi   = ($Sayfalama*$SYFBasinaGosterilenKayitSayisi) - $SYFBasinaGosterilenKayitSayisi;
$KacSayfaVar                        = ceil($KacTaneEvVar/$SYFBasinaGosterilenKayitSayisi);


$UrunSayisiBulmaQ        = $DataBaseConnection->query("SELECT * FROM konut WHERE Durumu = 1");
...
<table width="1065" align="center" border="0" cellpadding="0" cellspacing="0">
  <tr height="100">
          ...
          <tr>
              <td>
                  <table width="250" align="center" border="0" cellpadding="0" cellspacing="0">
                      ...
                      <tr>
                          <td>
                              &nbsp;&nbsp;<a href="Evler/1/" style="text-decoration: none;<?php if($GelenMenuID == ""){ ?> color: #FF7300; <?php }else{ ?>color: #9932CC;<?php } ?> font-weight: bold;">Tüm Menüler (<?php echo $KacTanesiOnline ?>)</a>
                          </td>
                      </tr>
	...
	function MenuYaz($MenuUstIdDegeri=0, $BoslukDegeri=0){
		...
		$MenuSorgusu			=	$DataBaseConnection->query("SELECT * FROM menuler WHERE TabloAdi = 'konut' AND ustID = $MenuUstIdDegeri AND UrunSayisi<>0");
		...
		if($MenuSorugusuSayi>0){
			while($MenuAdlari = $MenuSorgusu->fetch_assoc()){
				...
				$MenuMenuAdi	= EksidenArtiya(GeriDöndür($MenuAdlari["MenuAdi"]));
				...
				echo '<td><div><a href="Evler/1/' . $MenuAdlari["MenuAdi"] . '" style="text-decoration: none;' . $renk . 'font-weight: bold;">' . str_repeat("&nbsp;", $BoslukDegeri). $MenuMenuAdi . ' (' . $UrunSayisi . ')</a></div>';
                    }
          ...
          <table width="795" align="center" border="0" cellpadding="0" cellspacing="0">
              <tr>
                  <td>
                      <div class="AramaAlaniGenelDiv">
                          <form action="Evler/1/" method="post">
                              <?php if($GelenMenuID!=""){?>
                              ...
                              <div class="AramaAlaniYaziDiv"><input type="text" name="ArananKelime" class="AramaAlaniYAZI"></div>
                          </form>
                      </div>
                  </td>
              </tr>
              ...
              <tr>
                  <td>
                    <table width="795" align="center" border="0" cellpadding="0" cellspacing="0">
                        <?php
                    $KonutSayfasiQ             = $DataBaseConnection->query("SELECT * FROM konut WHERE Durumu = 1 $MenuKosulu $AramaKosulu ORDER BY id DESC LIMIT $SayfalamayaBaslanacakKayitSayisi, $SYFBasinaGosterilenKayitSayisi");
                    $OnlineKayitlar            = $KonutSayfasiQ->num_rows;

                    $dongusayisi=1;
                    $StunAdetSayisi=4;

                    while($tumkayitlariyaz = $KonutSayfasiQ->fetch_assoc()){
                    ...
                    <td width="191">
                        <table width="191" align="center" border="0" cellpadding="0" cellspacing="0" style=" border-bottom: dashed 1px #C500BE; margin-bottom: 10px;">
                            <tr>
                                <td valign="top" align="center"><a href="UrunDetaylari/<?php echo $UrunAdi; ?>/<?php echo "konut"; ?>/<?php echo $UrunID; ?>"><img src="<?php echo GeriDöndür($tumkayitlariyaz["UrunResmiBIR"]); ?>" width="191" height="247"></a></td>
                            </tr>
                            <tr height="30">
                                <td width="191" align="center" style="font-size: 12px;"><a href="UrunDetaylari/<?php echo $UrunAdi; ?>/<?php echo "konut"; ?>/<?php echo $UrunID; ?>" style="color: #C500BE; text-decoration: none;"><div style="width: 191px; max-width: 191px; height: 20px; overflow: hidden; line-height: 20px"><?php echo GeriDöndür($tumkayitlariyaz["UrunAdi"]); ?></div></a></td>
                            </tr>
                            <tr height="25">
                                <td width="191" align="center" style="font-size: 12px;"><a href="UrunDetaylari/<?php echo $UrunAdi; ?>/<?php echo "konut"; ?>/<?php echo $UrunID; ?>"><img src="Resimler/<?php echo $PuaninResmi ?>"></a></td>
                            </tr>

                            <tr height="25">
                                <td width="191" align="center" style="font-size: 12px;"><a href="UrunDetaylari/<?php echo $UrunAdi; ?>/<?php echo "konut"; ?>/<?php echo $UrunID; ?>" style="color: #9370DB; text-decoration: none;"><b><?php echo FiyatBicimlendir($UrunFiyatiHesapla); ?> TL</b></a></td>
                            </trkonut
                          ...
                          <td align="center">
                              ...
                              <div class="SayfalamaAlani">
                                  <div class="SayfalamaAlaniIciMetinAlani">
                                      Toplam <?php echo $KacSayfaVar; ?> sayfada, <?php echo $KacTaneEvVar; ?> adet Kayıt var.
                                  </div>
                                      <div class="SayfalamaAlaniIciNumaraAlani">
                                          <?php 
                                                if($Sayfalama > 1){
                                                echo "<span class='SayfalamaLinkiPasif'><a href='Evler/1" . $SYFileMenu . "'> << </a></span>";
                                                $sayfalamaEKSIbir = $Sayfalama - 1;
                                                echo "<span class='SayfalamaLinkiPasif'><a href='Evler/" . $sayfalamaEKSIbir . $SYFileMenu . "'> < </a></span>";
                                                }
                                                ...
                                                for($SYFicinSayfaIndexDegeri = $Sayfalama-$SYFSagSolButonSayisi; $SYFicinSayfaIndexDegeri<=$Sayfalama+$SYFSagSolButonSayisi; $SYFicinSayfaIndexDegeri++){
                                                    if(($SYFicinSayfaIndexDegeri>0) and ($SYFicinSayfaIndexDegeri<=$KacSayfaVar)){
                                                        if($Sayfalama == $SYFicinSayfaIndexDegeri){
                                                            ...
                                                            echo "<span class='SayfalamaLinkiPasif'><a href='Evler/" . $SYFicinSayfaIndexDegeri . $SYFileMenu . "'>" . $SYFicinSayfaIndexDegeri . "</a></span>";
                                                        }
                                                    }
                                                }
                                                ...
                                                echo "<span class='SayfalamaLinkiPasif'><a href='Evler/" . $sayfalamaARTIbir . $SYFileMenu . "'> > </a></span>";
                                                echo "<span class='SayfalamaLinkiPasif'><a href='Evler/" . $KacSayfaVar . $SYFileMenu . "'> >> </a></span>";
                                                }
                                          ?>
                                      </div>
                              </div>
